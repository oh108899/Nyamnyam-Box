...
export default function SearchResultsPage() {
  ...
  async function fetchResults(q: string) {
    ...
    try {
      ...
      let query = supabase.from("recipes").select("id,title,image,time,cooking_time,description").limit(50);
    ...
    <section className={styles.resultsSection}>
      <div className={styles.searchRow}>
        <input
          ...
          placeholder="레시피 검색"
        />
        <button className={styles.searchButton} onClick={handleSearch} disabled={loadingResults}>
          {loadingResults ? "검색중..." : "검색"}
        </button>
      </div>
